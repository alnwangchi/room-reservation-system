...
function RoomManage({ selectedRoomId = 'general-piano-room', onRoomChange }) {
  ...
  // 獲取時段顏色
  const getTimeSlotColor = category => {
    const colors = {
      morning: 'blue',
      afternoon: 'green',
      evening: 'purple',
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                {Object.keys(TIME_CATEGORIES).map(category => {
                  ...
                  const color = getTimeSlotColor(categoryKey);
                  ...
                  return (
                    <div
                      key={category}
                      className={`flex items-center justify-between p-3 bg-${color}-50 rounded-lg`}
                    >
                      ...
                      <Switch
                        ...
                        className={`${
                          timeSlots[categoryKey]
                            ? `bg-${color}-600`
                            : 'bg-gray-200'
                        } relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-${color}-500 focus:ring-offset-2`}
                      >
                        <span
                          className={`${
                            timeSlots[categoryKey]
